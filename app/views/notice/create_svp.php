...
$openingDateValue = isset($old['bid_submission_deadline']) ? str_replace(' ', 'T', substr((string) $old['bid_submission_deadline'], 0, 16)) : '';
?>
<div class="page-head">
    <div>
...
<form method="POST" action="<?= ViewHelper::escape(ResponseHelper::url('procurements/svp')); ?>" class="form-grid two-col">
    ...
    <div>
        <label for="bid_submission_deadline">Opening date</label>
        <input id="bid_submission_deadline" name="bid_submission_deadline" type="datetime-local" value="<?= ViewHelper::escape($openingDateValue); ?>" class="<?= ViewHelper::escape(ValidationHelper::inputClass($errors, 'bid_submission_deadline')); ?>" required>
