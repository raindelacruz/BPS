...
<dl class="detail-grid">
    ...
    <div>
        <dt>Date of posting</dt>
        <dd><?= ViewHelper::escape(!empty($bid['posting_date']) ? date('Y-m-d H:i', strtotime((string) $bid['posting_date'])) : '-'); ?></dd>
    </div>
    <div>
        <dt><?= (($bid['procurement_mode'] ?? $bid['mode_of_procurement'] ?? '') === 'svp') ? 'Opening date' : 'Bid submission deadline'; ?></dt>
        <dd><?= ViewHelper::escape(!empty($bid['bid_submission_deadline']) ? date('Y-m-d H:i', strtotime((string) $bid['bid_submission_deadline'])) : '-'); ?></dd>
    </div>
</dl>
