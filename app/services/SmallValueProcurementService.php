...
class SmallValueProcurementService extends BaseService
{
    ...
    public function validateParentInput(array $input): array
    {
        ...
        foreach (['procurement_title', 'reference_number', 'abc', 'description', 'category', 'end_user_unit', 'posting_date', 'bid_submission_deadline'] as $field) {
            if ($data[$field] === '') {
        ...
        if ($data['posting_date'] !== '' && $data['bid_submission_deadline'] !== '') {
            try {
                ...
                $openingDate = new DateTimeImmutable($data['bid_submission_deadline']);
                if ($openingDate <= $postingDate) {
                    ValidationHelper::addError($errors, 'bid_submission_deadline', 'Opening date must be later than the posting date.');
                }
                $data['posting_date'] = $postingDate->format('Y-m-d H:i:s');
                $data['bid_submission_deadline'] = $openingDate->format('Y-m-d H:i:s');
            } catch (\Exception) {
                ValidationHelper::addError($errors, 'posting_date', 'Posting date or opening date is invalid.');
            }
        }
